...
namespace Microsoft_Graph_SDK_ASPNET_Connect.Models
{
    public class GraphService
    {
        ...
        public async Task<string> GetMyPhoto(GraphServiceClient graphClient)
        {
            // Get the current user's photo. 
            try
            {
                Stream photoStream = await graphClient.Me.Photo.Content.Request().GetAsync();
                byte[] byteData = new byte[photoStream.Length];
                photoStream.Position = 0;
                photoStream.Read(byteData, 0, (int)photoStream.Length);
                photoStream.Close();
                string imgBased64Data = Convert.ToBase64String(byteData);
                return string.Format("data:image/jpeg;base64,{0}", imgBased64Data);
            }
            catch (ServiceException)
            {
                return null;
